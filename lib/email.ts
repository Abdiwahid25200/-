 ...
 *
 * ⚠️ **والاسمُ الظاهر `eramaan.com` لا `Ramaan`** — قرارها (٠٨-٠٨):
 *    «بدي يطلع eramaan.com». صندوقُ الوارد يعرض الاسمَ لا العنوان،
 *    و«Ramaan» كلمةٌ قد تشبه غيرَها، أمّا النطاق فعنوانُ متجرها الذي
 *    تكتبه في كل مكان — يُقرأ فيُعرف من أوّل نظرة.
 *
 * ⚠️ **وبين علامتَي اقتباس**: الاسمُ فيه نقطة، والنقطةُ في اسمٍ غير
 *    مقتبَس مخالفةٌ لصيغة البريد القياسية — يقبلها بعضُ المزوّدين
 *    ويرفضها بعضٌ، والاقتباسُ يجعلها صحيحةً عند الجميع.
 */
const FALLBACK_FROM = '"eramaan.com" <onboarding@resend.dev>';
