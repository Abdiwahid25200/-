...
/**
 * 🔒 **عمر الخَتْم داخل القواعد** — قرارها (٠٧-٠٨): **ساعةٌ واحدة**.
 *
 * وهذا هو الفرق بين بوّابةٍ في الشاشة وبوّابةٍ في البيانات: القواعد كانت
 * تسأل «هل هو أدمن؟» وحدها، فمن عرف كلمة السرّ وخاطب Firebase رأساً
 * دخل بلا رمز. صارت تسأل: «**وهل خَتْمُه حيّ؟**» — والخَتْم لا يكتبه إلا
 * حسابُ البوّاب من خادمنا بعد الرمز الصحيح.
 *
 * ⚠️ **وقفلُ الجلسة ثلاثون دقيقة** (`lib/adminSession.ts`) أقصرُ من
 *    الساعة، والتجديد الصامت كل عشرين دقيقة — فلا ينتهي الخَتْم وهي
 *    داخل اللوحة أبداً، وينتهي وحده بعد أن تتركها.
 */
export const STAMP_MS = 60 * 60_000;

/** كل كم يجدّد المتصفّح الخَتْم وهو مفتوح — أقصرُ من عمره بثلثه */
export const STAMP_BEAT_MS = 20 * 60_000;
