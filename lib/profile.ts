...
export type Profile = {
  ...
  email: string;
  /**
   * رصيد بَرْواقو — يُقرأ من وثيقته نفسها فلا قراءةَ ثانية لعرضه في
   * صفّ «حسابي». 🔒 والكتابة فيه للأدمن وحده في `firestore.rules`،
   * فوجودُه هنا عرضٌ لا صلاحية.
   */
  points?: number;
};
...
export async function saveProfile(
  ...
  try {
    ...
    /**
     * دليل الأرقام: وثيقةٌ اسمها الرقم وفيها صاحبه.
     * ⚠️ **لماذا؟** لأن التحقّق من وجود حساب برقمٍ ما لا يمكن أن يمرّ
     *    على `users` — لو سُمح بالبحث فيها لقرأ أي زبون أرقام الزبائن
     *    كلّها. هنا تُقرأ وثيقةٌ **بالرقم المحدَّد وحده**، ولا تُسرد
     *    المجموعة أبداً، فلا يُكشف إلا ما سأل عنه سائلٌ يعرفه سلفاً.
     */
    await setDoc(
      doc(db, "phones", phone.replace(/\D/g, "")),
      { uid: user.uid },
      { merge: true },
    ).catch(() => {});
