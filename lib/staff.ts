...
export type StaffDoc = {
  ...
  active?: boolean;
  /**
   * 🔑 **بريد المساعد الحقيقيّ** — إليه يُرسَل رمز الدخول.
   *
   * ⚠️ **ولماذا حقلٌ مستقلّ عن `email`**: الأوّل اسمُ الوثيقة وهو بريدُ
   *    دخولٍ داخليّ (`sara@eramaan.com`) لا صندوق له — حيلةٌ ليدخل باسمٍ
   *    يحفظه. فلو أُرسل الرمز إليه لَذهب إلى لا مكان.
   *
   * ⚠️ **ولازمٌ عند الإنشاء لا زينة**: بلا بريدٍ لا رمز، وبلا رمزٍ لا
   *    يدخل من جهازٍ جديد. وهو أيضاً طريقُك إليه حين يتعثّر أو يغيب.
   */
  mail?: string;
  /**
   * رقم واتساب — **حقلٌ قديم** (كان الرمز يُرسَل إليه قبل أن يصير
   * بالبريد بقرارها ٠٣-٠٨). يبقى في النوع ليُقرأ من وثائق قديمة، ولا
   * تكتبه الشاشة بعد اليوم.
   */
  phone?: string;
};

/** أرقامٌ فقط — من كتب `+252 61 5` تُحفظ `252615` */
export const digitsOnly = (v: string) => String(v ?? "").replace(/\D/g, "");

/** رقمٌ مقبول: تسع خاناتٍ فأكثر مع مفتاح الدولة */
export const validStaffPhone = (v: string) => digitsOnly(v).length >= 9;

/**
 * بريدٌ مقبول شكلاً — وحارسٌ فوقه: **لا يكون بريد الدخول الداخليّ نفسه**.
 * من كتب `sara@eramaan.com` هنا لم يكتب صندوقاً، بل أعاد كتابة الاسم.
 */
export const validStaffMail = (v: string) => {
  const s = String(v ?? "").trim().toLowerCase();
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s) && !s.endsWith("@eramaan.com");
};
